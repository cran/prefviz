...
- `get_tern_data2d()`: Provides input data for `ggplot2` (2D ternary plots).
- `get_tern_datahd()`: Provides input data for `tourr` (high-dimensional ternary plots). The returned
  data frame includes a `labels` column containing vertex names for simplex rows and `""` for
  observation rows, which can be passed directly to `tourr`'s `obs_labels` argument.
- `get_tern_edges()`: Provides edges of the simplex, which is required by `tourr`.

Note: `get_tern_labels()` is deprecated. Use `get_tern_datahd(tern)[["labels"]]` instead.
...
Now we can use the `get_tern_data2d()` function to get the input data for `ggplot2`.
